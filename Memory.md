...
- Primary implementation: signed `CuaDriver.app` 0.12.6, launched on a dedicated socket in unrestricted mode for background AX/pixel control without focus stealing.
- Fallback implementation: repository-owned Python/PyObjC MCP using AppKit, Quartz, and macOS Accessibility for direct foreground control when the driver is unavailable or refuses an operation.
...
- Add raw cursor, mouse button, clipboard, health, and permission tools beyond the compatibility core.
...
- Pin the upstream installer by release tag and SHA-256; reuse a compatible existing driver rather than blindly replacing it.
