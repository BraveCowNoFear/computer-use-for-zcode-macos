...
- 可把某个具名会话临时切到不缩放的原始窗口 PNG，用于逐像素核验；同时证明并发会话保留
  自己的有效配置，完成后恢复，不改守护进程的全局持久默认值。
...
兜底，检查固定发布归档与 Cua AI 签名身份，直接解析该签名二进制的九个类型化浏览器
请求 schema，并运行真实 Plugin 自有首次安装。真实的后台
“截图 → 点击/输入 → 再截图”
