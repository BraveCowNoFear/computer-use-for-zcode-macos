...
- 可先快速盘点正在运行的 App 与可见窗口而不读取窗口内容，再把一个真实返回的
  pid/窗口绑定到完整截图加辅助功能循环。这个读取只使用 AppKit/WindowServer，
  不弹 TCC；其文字摘要必须由同一份结构化身份逐行精确生成，不是第二套目标真相。
...
- 可按用户明确要求在本地记录带顺序的动作轨迹、前后截图、状态和参数。上游录制是守护进程
  全局状态：新启动会接管 owner，手动停止也无条件；因此要先查状态、避免覆盖另一任务，默认不录视频，
  并在读取证据前结束自己的精确目录。同一存活窗口内的
...
应用/窗口/屏幕/光标发现；两条 MCP 连接都必须返回精确的轻量 Accessibility 清单与
对应文字摘要，并拒绝重复或无效的 App/窗口身份；在仅写入临时本地
`session.json` 的前提下完成自有无视频录制的启动、双连接读回与停止，真实证明第二条
连接会接管全局 owner、旧连接的手动停止仍会无条件结束它；再通过 ZCode
...
门禁才算通过。主会话状态机也会完整执行：同 scope 重复启动保持幂等，活动期改 scope
返回精确且无副作用的冲突，`auto` 只能带真实原因单向升级，结束后状态不可见，而显式
...
配置响应也已端到端锁定：七字段读取形状、正式归档身份与默认 PiP 字段、两种图像上限
setter 形状、精确的连接内确认、并行连接隔离、恢复，以及退役 `capture_scope` 的结构化
无副作用错误都会被验证。
权限预检响应也被完整锁定：空参数与显式只读调用必须一致，source 身份必须绑定到实时
签名 daemon 的 pid/路径/bundle，文本摘要必须匹配布尔状态，`prompt:true` 必须保持精确的
trusted-host TCC 拒绝。
新会话的语义光标响应也会被精确锁定：完整状态、内嵌 `cua.default` 主题身份、九个
拟人运动参数、空闲或仍在播放的有界视觉语义、幂等主题/运动 setter、可见性切换与
恢复后读回必须一致，整个验证不移动真实鼠标，也不依赖 TCC。
...
不会假设裸 Python 进程具备完整的 `.app` AX 树。两条路径都会重新观察可见结果。为容忍真实前台焦点竞争而不降低结果门槛，
兜底链会在新的像素观察后，通过夹具原子发布的本地字段状态确认文字已经进入；若没有，则重新观察、重新聚焦并只重输一次，
不会假设裸 Python 进程能通过 AX 暴露字段值。首次物理提交点击若没有本地可见结果，
它会再取一张截图、重算绑定坐标并只重试一次；第二次仍无结果就让门禁失败。结束前会恢复原光标位置并自动关闭测试窗口，不接触用户文档：
